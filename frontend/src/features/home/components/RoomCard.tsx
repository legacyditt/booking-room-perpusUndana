import { Users, CheckCircle, WifiHigh, Monitor, ProjectorScreen, Chalkboard } from "@phosphor-icons/react/dist/ssr";
...
function renderFeatureIcon(iconName: string) {
  switch (iconName) {
    case "wifi": return <WifiHigh className="h-4 w-4" />;
    case "monitor": return <Monitor className="h-4 w-4" />;
    case "projector": return <ProjectorScreen className="h-4 w-4" />;
    case "whiteboard": return <Chalkboard className="h-4 w-4" />;
    ...
    <Card className="overflow-hidden border-border bg-white shadow-sm transition-all hover:shadow-md">
      ...
      <div className="relative aspect-[4/3] w-full bg-muted">
        <img 
          ...
          onError={(e) => {
            e.currentTarget.src = "https://placehold.co/600x400/e2e8f0/4a4a4a?text=Room+Image";
          }}
        />
        <Badge 
          variant="outline"
        ...
        >
          <CheckCircle weight="fill" className={room.isAvailable ? "text-green-600" : "text-neutral"} />
      ...
      <CardHeader className="space-y-2 pb-4">
        <h3 className="font-serif text-2xl font-bold text-primary">{room.name}</h3>
